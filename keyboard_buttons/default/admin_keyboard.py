...
class Database:
    ...
    def add_voice(self, name:str, voice:str):

        sql = """
        INSERT INTO voice(name, voice) VALUES(?, ?);
        """

        self.execute(sql, parameters=(name, voice), commit=True)
    ...
    async def search_voices_title(self, title: str):
        sql = f"""
        SELECT * FROM voice WHERE title LIKE "%{title}%"
        """
        return self.execute(sql, fetchall=True)
